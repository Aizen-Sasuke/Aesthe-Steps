  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-[#FAF6F0] border-l border-pink-200 text-[#18181B] shadow-2xl flex flex-col justify-between">
          
          {/* Drawer Header */}
          <div className="p-5 border-b border-pink-200/80 flex items-center justify-between bg-[#FDF2F4]">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-[#DB2777]" />
              <h2 className="text-lg font-bold font-serif-display tracking-tight text-[#18181B]">
                Your Aesthé Bag
              </h2>
              <span className="text-xs bg-white text-[#DB2777] border border-pink-200 px-2.5 py-0.5 rounded-full font-bold">
                {cartItems.length} {cartItems.length === 1 ? 'item' : 'items'}
              </span>
            </div>
            <button
              onClick={onClose}
              className="p-2 rounded-full text-[#18181B]/60 hover:text-[#18181B] hover:bg-pink-100/60 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6">
            ...
            {orderPlaced ? (
              <div className="text-center py-10 space-y-4">
                ...
                <h3 className="text-xl font-bold font-serif-display text-[#18181B]">Order Confirmed!</h3>
                <p className="text-xs text-[#18181B]/70 max-w-xs mx-auto">
                  Thank you, <span className="text-[#18181B] font-bold">{customerName}</span>! Your Aesthé Steps order <span className="text-[#DB2777] font-mono font-bold">{orderId}</span> is being prepared for dispatch in Dhaka.
                </p>
                <div className="p-4 rounded-2xl bg-white border border-pink-200 text-left space-y-2 text-xs shadow-2xs">
                  <div className="flex justify-between text-[#18181B]/70">
                    <span>Payment Method:</span>
                    <span className="text-[#18181B] font-semibold uppercase">{paymentMethod}</span>
                  </div>
                  <div className="flex justify-between text-[#18181B]/70">
                    <span>Total Payable:</span>
                    <span className="text-[#18181B] font-bold">৳ {totalBDT.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-[#18181B]/70">
                    <span>Delivery Address:</span>
                    <span className="text-[#18181B] text-right font-medium">{customerAddress}</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-3 rounded-full bg-[#18181B] hover:bg-[#F472B6] text-white font-bold text-xs uppercase tracking-wider transition-colors"
                >
                  Continue Shopping
                </button>
              </div>
              ...
              <div className="text-center py-16 space-y-4">
                <div className="w-16 h-16 rounded-full bg-white border border-pink-200 text-[#18181B]/40 flex items-center justify-center mx-auto shadow-2xs">
                  <ShoppingBag className="w-7 h-7" />
                </div>
                <h3 className="text-base font-bold text-[#18181B] font-serif-display">Your bag is empty</h3>
                <p className="text-xs text-[#18181B]/60 max-w-xs mx-auto">
                  Pick your favorite slides from Drop 01 to checkout.
                </p>
                <button
                  onClick={onClose}
                  className="px-6 py-2.5 rounded-full bg-[#18181B] text-white hover:bg-[#F472B6] text-xs font-bold transition-all uppercase tracking-wider"
                >
                  Explore Drops
                </button>
              </div>
              ...
              <>
                ...
                <div className="p-4 rounded-2xl bg-white border border-pink-200/80 space-y-3 shadow-2xs">
                  <div className="flex items-center justify-between text-xs font-semibold text-[#18181B]">
                    <span className="flex items-center gap-1.5 font-bold">
                      <Truck className="w-4 h-4 text-[#DB2777]" /> Bangladesh Delivery
                    </span>
                    {subtotalBDT > 4000 && (
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-left transition-all ${
                        deliveryArea === 'inside_dhaka'
                          ...
                          : 'border-pink-200 bg-[#FAF6F0] text-[#18181B]/70 hover:text-[#18181B]'
                      }`}
                    >
                      <div className="font-bold">Inside Dhaka</div>
                      <div className="text-[10px] text-[#18181B]/60">24-48h • ৳ 70</div>
                    </button>
                    ...
                    >
                      ...
                      <div className="text-[10px] text-[#18181B]/60">48-72h • ৳ 120</div>
                    </button>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white border border-pink-200 rounded-xl px-3 py-2 text-xs text-[#18181B] placeholder-[#18181B]/40 focus:outline-none focus:border-[#F472B6] uppercase"
                  />
                  <button
                    type="button"
                    onClick={handleApplyPromo}
                    className="px-4 py-2 rounded-xl bg-[#18181B] hover:bg-[#F472B6] text-xs font-bold text-white transition-colors uppercase tracking-wider"
                  >
                    Apply
                  </button>
                </div>
                ...
                {appliedPromo && (
                  <div className="text-[11px] text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg flex justify-between items-center font-medium">
                    <span>Coupon '{appliedPromo}' applied!</span>
                    <button onClick={() => setAppliedPromo(null)} className="text-[#18181B]/60 hover:text-[#18181B]">
                      ✕
                    </button>
                  </div>
                )}
                ...
                <form id="checkout-form" onSubmit={handlePlaceOrder} className="space-y-3 pt-2">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[#18181B]/70">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-pink-200 rounded-xl px-3 py-2 text-xs text-[#18181B] placeholder-[#18181B]/40 focus:outline-none focus:border-[#F472B6]"
                  />
                  ...
                  <div className="space-y-1.5 pt-1">
                    <label className="text-[11px] font-bold text-[#18181B]/70 uppercase tracking-wider">Payment Option</label>
                    <div className="grid grid-cols-3 gap-2 text-[11px]">
                      <button
                        ...
                        className={`p-2 rounded-xl border text-center font-bold ${
                          paymentMethod === 'cod'
                            ? 'border-[#F472B6] bg-[#FDF2F4] text-[#DB2777]'
                            : 'border-pink-200 bg-white text-[#18181B]/70'
                        }`}
                      >
                        Cash on Delivery
                      </button>
          ...
          {!orderPlaced && cartItems.length > 0 && (
            <div className="p-5 border-t border-pink-200/80 bg-white space-y-3">
              <div className="space-y-1 text-xs text-[#18181B]/70">
                <div className="flex justify-between">
                  <span>Subtotal:</span>
                  <span className="text-[#18181B] font-bold">৳ {subtotalBDT.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between text-sm font-bold text-[#18181B] pt-2 border-t border-pink-100">
                  <span>Total Payable:</span>
                  <span className="text-[#DB2777] text-base font-bold">৳ {totalBDT.toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 text-[11px] text-[#18181B]/60 font-medium">
